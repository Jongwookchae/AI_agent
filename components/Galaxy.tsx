...
export default function Galaxy({
  ...
  useEffect(() => {
    ...
    if (mouseInteraction) {
      // Use window listeners so interaction still works when the Galaxy layer has pointer-events disabled.
  ...
  return (
    <div
      ref={ctnDom}
      className={`galaxy-container absolute inset-0 ${className ?? ""}`}
      {...rest}
    />
  );
}
